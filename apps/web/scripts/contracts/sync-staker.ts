 ...
 *
 * Usage:
 *   npx tsx apps/web/scripts/contracts/sync-staker.ts <WALLET_ADDRESS>
 *
 * Or sync all stakers:
 *   npx tsx apps/web/scripts/contracts/sync-staker.ts --all
 */
...
async function main() {
    ...
    if (args.length === 0) {
        console.error('Usage: npx tsx apps/web/scripts/contracts/sync-staker.ts <WALLET_ADDRESS>');
        console.error('   Or: npx tsx apps/web/scripts/contracts/sync-staker.ts --all');
        process.exit(1);
    }
