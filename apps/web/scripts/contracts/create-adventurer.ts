 ...
 * Usage: pnpm tsx scripts/contracts/create-adventurer.ts <tokenId> [walletAddress] [class] [name]
 */
...
async function createAdventurer() {
  ...
  if (args.length < 1) {
    console.log('Usage: pnpm tsx scripts/contracts/create-adventurer.ts <tokenId> [walletAddress] [class] [name]');
    console.log('');
    console.log('Example:');
    console.log('  pnpm tsx scripts/contracts/create-adventurer.ts 6 0x1234... warrior "Hero #6"');
    process.exit(1);
  }
