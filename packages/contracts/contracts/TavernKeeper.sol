...
contract TavernKeeper is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function initializeOfficeV2(address _treasury) public onlyOwner {
        ...
        lastOfficeClaimTimestamp = block.timestamp; // Initialize

        // Migrate legacy state if exists
        if (currentKing != address(0)) {
            slot0.miner = currentKing;
        } else {
    ...
    function takeOffice(
        ...
        if (lastOfficeClaimTimestamp > startTime) {
            startTime = lastOfficeClaimTimestamp;
        }

        if (block.timestamp > startTime) {
            uint256 mineTime = block.timestamp - startTime;
    ...
    function claimOfficeRewards() public nonReentrant {
        ...
        require(block.timestamp > startTime, "No time passed");
        ...
        lastOfficeClaimTimestamp = block.timestamp;

        IKeepToken(keepToken).mint(msg.sender, minedAmount);
        emit OfficeRewardsClaimed(msg.sender, minedAmount);
    }
    ...
    function _getPriceFromCache(Slot0 memory slot0Cache) internal view returns (uint256) {
        ...
        uint256 priceDrop = slot0Cache.initPrice * timePassed / EPOCH_PERIOD;
        uint256 currentPrice = slot0Cache.initPrice - priceDrop;

        return currentPrice < MIN_INIT_PRICE ? MIN_INIT_PRICE : currentPrice;
    }
    ...
    // Helper to get pending rewards for UI
    function getPendingOfficeRewards() external view returns (uint256) {
        ...
        return (block.timestamp - startTime) * slot0Cache.dps;
    }
