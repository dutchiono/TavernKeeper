...
The following migrations from game-engine need to be run to enable full game functionality:

## Required Migrations

1. **Adventurer Tracking** (`apps/web/game-engine/adventurer-tracking/code/database/migration.sql`)
   - Creates `adventurers` table
   - Creates `adventurer_stat_history` table
   - Required for hero stat tracking

2. **Inventory Tracking** (`apps/web/game-engine/inventory-tracking/code/database/migration.sql`)
...
3. **Game Logging System** (`apps/web/game-engine/game-logging-system/code/database/migration.sql`)
   - Creates `detailed_logs` table
   - Creates `key_events` table
   - Required for event logging during dungeon runs

4. **World Content Hierarchy** (`apps/web/game-engine/world-content-hierarchy/code/database/migration.sql`)
