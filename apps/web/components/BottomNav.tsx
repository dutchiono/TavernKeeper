...
export const BottomNav: React.FC = () => {
    ...
    return (
        <nav className="h-16 bg-[#2a1d17] border-t-4 border-[#eaddcf] flex justify-around items-center px-2 z-50 shrink-0 shadow-[0_-4px_10px_rgba(0,0,0,0.5)]">
            <button
            ...
            <button
                onClick={() => handleNavigation(GameView.CHAT)}
                className="flex flex-col items-center gap-1 p-2 group"
            >
                <div className={`w-10 h-10 border-2 rounded-sm flex items-center justify-center text-xl shadow-md transition-all group-hover:-translate-y-1 ${isActive(GameView.CHAT) ? 'bg-[#5c4b40] border-[#eaddcf]' : 'bg-[#4a3b32] border-[#855e42] group-hover:border-[#eaddcf] group-hover:bg-[#5c4b40]'}`}>
                    💬
                </div>
                <span className={`text-[8px] uppercase font-bold tracking-widest drop-shadow-md ${isActive(GameView.CHAT) ? 'text-white' : 'text-[#eaddcf] group-hover:text-white'}`}>Chat</span>
            </button>
        </nav>
    );
};
