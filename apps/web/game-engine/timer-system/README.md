...
The Timer System manages time-based event delivery for dungeon runs. Events are calculated deterministically immediately, but are delivered to the frontend over time at 3-second intervals.
...
4. **Sequential Scheduling**: Events are scheduled with 3-second intervals
...
```typescript
import { scheduleEvent, scheduleEventsSequentially } from '@/game-engine/timer-system/code/services/timerService';
...
const event = await scheduleEvent({
  ...
  delaySeconds: 3, // Deliver 3 seconds from now
});
...
```typescript
import { getEventsReadyToDeliver } from '@/game-engine/timer-system/code/services/timerService';
...
```typescript
import { markEventsAsDelivered } from '@/game-engine/timer-system/code/services/timerService';
